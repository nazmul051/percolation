...
public class Percolation {
    ...
    public Percolation(int n){
        this.totalRows = n;
        this.N = (n*n) + 2;
        this.statesOfSites = new boolean[this.N]; //all initialized to false
        this.sites = new WeightedQuickUnionUF(this.N);
    ...
    public void open(int row, int col){
        if(row<1 || col<1 || row>this.totalRows || col>this.totalRows)
        ...
        else if(this.statesOfSites[((row - 1)*this.totalRows) + col] == false){
            ...
            if(row==1){
                ...
                this.sites.union(element, this.N-1);
                if(this.isOpen(row-1, col) == true){
    ...
    public boolean percolates(){
        ...
        if(this.sites.find(0) == this.sites.find(this.N-1)) return true;
        else return false;
    }
